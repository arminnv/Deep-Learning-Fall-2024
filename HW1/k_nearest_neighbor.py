...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        """
        Nte = X.shape[0]
        Ntr = self.X_train.shape[0]
        dists = np.zeros((Nte, Ntr))

        for i in range(Nte):
            for j in range(Ntr):
                # Compute the L2 distance between the i-th test point and the j-th training point
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            # Broadcasting to calculate L2 distance between ith test point and all training points
    ...
    def compute_distances_no_loops(self, X):
        ...
        Input / Output: Same as compute_distances_two_loops
        """
        # Breaking down the formula (a - b)^2 = a^2 + b^2 - 2*a*b
        test_sum = np.sum(X ** 2, axis=1).reshape(-1, 1)    # Shape (num_test, 1)
        train_sum = np.sum(self.X_train ** 2, axis=1)       # Shape (num_train,)
        inner_product = X.dot(self.X_train.T)               # Shape (num_test, num_train)

        # Combining terms to get (a - b)^2 and taking the square root
